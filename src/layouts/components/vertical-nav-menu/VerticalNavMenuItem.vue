    ...
    <div
      ...
      {'disabled-item pointer-events-none' : isDisabled}
        ...
        <router-link
                ...
                :to="to"
                :target="target">
            <vs-icon v-if="!featherIcon" :class="{'w-3 h-3': iconSmall}" :icon-pack="iconPack" :icon="icon"/>
            <feather-icon v-else :class="{'w-3 h-3': iconSmall}" :icon="icon"/>
            <slot/>
        </router-link>

        <a v-else :target="target" :href="href" tabindex="-1">
            <vs-icon v-if="!featherIcon" :icon-pack="iconPack" class="font-s-icon"
...
<style scoped>
    ...
    .v-nav-menu .vs-sidebar--item .router-link-active .vs-icon {
        color: white;
    }
</style>
